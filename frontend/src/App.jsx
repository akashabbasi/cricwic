...
function App() {
  ...
  useEffect(() => {
    const cachedKey = sessionStorage.getItem("stripeApiKey");
    if (cachedKey) {
      setStripeApiKey(cachedKey);
    } else {
      getStripeApiKey();
    }
    // eslint-disable-next-line
  ...
  return (
    <>
      <Router>
        ...
        {/* ⚡ Safety Check: Only execute loadStripe when stripeApiKey holds a valid string */}
        {stripeApiKey && (
          <Elements stripe={loadStripe(stripeApiKey)}>
            <Switch>
              <Route exact path="/process/payment">
